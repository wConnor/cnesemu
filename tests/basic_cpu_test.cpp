...
TEST(basic_cpu_test, load_store)
{
	...
	// STX ABS: stores the contents of the X register into the desired memory location.
	(*mem)[0x0009] = 0x8E;
	(*mem)[0x000A] = 0xAB;
	(*mem)[0x000B] = 0xBB;

	// STY ABS: stores the contents of the Y register into the desired memory location.
	(*mem)[0x000C] = 0x8C;
	(*mem)[0x000D] = 0xAC;
	(*mem)[0x000E] = 0xBB;
...
TEST(basic_cpu_test, logical)
{
	...
	// randomise accumulator for testing.
	std::uint8_t temp = std::rand() % std::numeric_limits<std::uint8_t>::max(); // randomise ACC value.
	cpu->acc = temp;

	// AND ABS: logical and is performed, bit-by-bit, using the contents of memory.
	(*mem)[0x0000] = 0x2D;
	(*mem)[0x0001] = 0x33;
	(*mem)[0x0002] = 0x44;
	(*mem)[0x4433] = 0xFF; // byte to AND w/ ACC.
	...
	EXPECT_EQ(cpu->acc & 0b10000000, (cpu->sr) & 0b10000000); // check N flag has been set correctly.

	std::uint8_t before = cpu->acc;

	// EOR IMM: exclusive OR operation is performed bit-by-bit on the ACC and contents of memory.
	(*mem)[0x0003] = 0x49;
	(*mem)[0x0004] = 0x55;
	...
	EXPECT_EQ(cpu->acc, before ^ (*mem)[0x0004]); // check AND on ACC has been carried out.
	EXPECT_EQ(cpu->acc & 0b10000000, (cpu->sr) & 0b10000000); // check N flag has been set correctly.
}
